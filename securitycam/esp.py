...
def feature_color(key: str, fallback_hex: str, camera_state) -> Tuple[int, int, int]:
    """Resolve per-feature color with fallback to provided hex."""
    return hex_to_bgr(camera_state.esp_settings.get(key, fallback_hex))
...
def draw_custom_esp(camera_state, frame, results):
    """
    ...
    if camera_state.esp_settings["thermal"]:
        frame[:] = cv2.applyColorMap(gray, cv2.COLORMAP_INFERNO)

    if camera_state.esp_settings["gay_mode"]:
        col_primary = (0, 255, 0)
        col_secondary = (0, 255, 255)
    else:
        col_primary = hex_to_bgr(camera_state.esp_settings.get("color_primary", "#00ff00"))
        col_secondary = hex_to_bgr(camera_state.esp_settings.get("color_secondary", "#ff00ff"))

    if camera_state.esp_settings["center_dot"]:
        c_col = get_rainbow_color(0.5) if camera_state.esp_settings["gay_mode"] else (0, 0, 255)
        l = 20
    ...
    now = time.time()
    line_thickness = max(1, int(round(camera_state.esp_settings.get("line_thickness", 2))))
    ...
    for i, box in enumerate(boxes):
        ...
        if camera_state.esp_settings["person_only"] and cls_name.lower() != "person":
            continue
    ...
    closest_idx = None
    if camera_state.esp_settings["highlight_center"]:
    ...
    faces = camera_state.last_faces
    face_det_scale = camera_state.esp_settings.get("face_zoom_scale", 2.0)
    if camera_state.esp_settings.get("face_boxes") or camera_state.esp_settings.get("face_zoom") or camera_state.esp_settings.get("face_blur"):
        if camera_state.stats["total_frames"] % config.FACE_DETECT_EVERY == 0:
            ...
            if cascade is not None:
                try:
                    ...
                    faces = camera_state.last_faces
            camera_state.last_faces[:] = faces
            camera_state.last_face_frame = camera_state.stats["total_frames"]
    camera_state.stats["faces"] = len(faces)

    # Hand detection (MediaPipe) throttled
    hands_landmarks = camera_state.last_hands
    if camera_state.esp_settings.get("hand_skeleton"):
        if camera_state.stats["total_frames"] % config.HAND_DETECT_EVERY == 0:
            detector = deps.ensure_hand_detector(camera_state)
            if detector:
                try:
                    ...
                    pass
            camera_state.last_hands[:] = hands_landmarks
            camera_state.last_hand_frame = camera_state.stats["total_frames"]

    current_frame_idx = camera_state.stats.get("total_frames", 0)
    ...
    for det_i, det in enumerate(detections):
        ...
        if key in camera_state.smoothed_boxes:
            prev = camera_state.smoothed_boxes[key]
        ...
        camera_state.smoothed_boxes[key] = {
            "x1": x1, "y1": y1, "x2": x2, "y2": y2,
            "bx": bx, "by": by, "conf": conf, "last": current_frame_idx
        }

        if obj_id != -1:
            if obj_id not in camera_state.track_history:
                camera_state.track_history[obj_id] = []
            camera_state.track_history[obj_id].append((bx, by, now))
            if len(camera_state.track_history[obj_id]) > camera_state.max_track_history:
                camera_state.track_history[obj_id].pop(0)

        if camera_state.esp_settings["gay_mode"]:
            color = get_rainbow_color(offset=det_i * 0.1 + obj_id * 0.2, speed=0.5)
            text_color = (255, 255, 255)
        elif camera_state.esp_settings["class_colors"]:
            color = class_id_to_bgr(cls_id)
            text_color = (255, 255, 255)
        else:
            color = feature_color("color_boxes", camera_state.esp_settings.get("color_primary", "#00ff00"), camera_state)
            text_color = feature_color("color_text", camera_state.esp_settings.get("color_primary", "#00ff00"), camera_state)
        ...
        if camera_state.esp_settings["snaplines"]:
            cv2.line(frame, (center_x, h), (bx, y2), color, line_thickness, cv2.LINE_AA)

        if obj_id != -1 and obj_id in camera_state.track_history:
            hist = camera_state.track_history[obj_id]
            if camera_state.esp_settings["tracers"] and len(hist) > 1:
                tracer_col = feature_color("color_tracers", camera_state.esp_settings.get("color_secondary", "#ff00ff"), camera_state)
                pts = np.array([[pt[0], pt[1]] for pt in hist], np.int32)
                pts = pts.reshape((-1, 1, 2))
                cv2.polylines(frame, [pts], False, tracer_col, line_thickness)

            if (camera_state.esp_settings["velocity"] or camera_state.esp_settings["prediction"]) and len(hist) >= 5:
                subset = hist[-5:]
                ...
                if dt > 0:
                    vx = dx / dt
                    vy = dy / dt
                    if camera_state.esp_settings["velocity"]:
                        vel_col = feature_color("color_velocity", camera_state.esp_settings.get("color_secondary", "#ff00ff"), camera_state)
                        end_x = int(bx + vx * 0.5)
                        end_y = int(by + vy * 0.5)
                        cv2.arrowedLine(frame, (bx, by), (end_x, end_y), vel_col, line_thickness, tipLength=0.3)
                    if camera_state.esp_settings["prediction"]:
                        pred_col = feature_color("color_prediction", camera_state.esp_settings.get("color_secondary", "#ff00ff"), camera_state)
        ...
        if camera_state.esp_settings["boxes"]:
            if camera_state.esp_settings["box_style"] == "corner":
                l = int((x2 - x1) * 0.2)
                t = line_thickness
            ...
            elif camera_state.esp_settings["box_style"] == "3d":
            ...
            if camera_state.esp_settings.get("fill_box", False):
                overlay = frame.copy()
                fill_col = color if (camera_state.esp_settings["class_colors"] or camera_state.esp_settings["gay_mode"]) else col_secondary
                cv2.rectangle(overlay, (x1, y1), (x2, y2), fill_col, -1)
                cv2.addWeighted(overlay, 0.18, frame, 0.82, 0, frame)
            if camera_state.esp_settings.get("chams", False):
                overlay = frame.copy()
                cham_col = feature_color("chams_color", "#00ffff", camera_state)
                cv2.rectangle(overlay, (x1, y1), (x2, y2), cham_col, -1)
                alpha = float(camera_state.esp_settings.get("chams_opacity", 0.35))
                alpha = max(0.0, min(alpha, 1.0))
                cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)

        label_parts = []
        if camera_state.esp_settings["tracking"] and obj_id != -1:
            label_parts.append(f"ID:{obj_id}")
        if camera_state.esp_settings["names"]:
            label_parts.append(cls_name.upper())
        if camera_state.esp_settings["distance"]:
            h_px = max(y2 - y1, 1)
            dist_m = 1000.0 / h_px
            label_parts.append(f"{dist_m:.1f}m")
        if camera_state.esp_settings["conf"]:
        ...
        if (camera_state.esp_settings["skeleton"] or camera_state.esp_settings["head"]) and keypoints is not None and len(keypoints.xy) > det["idx"]:
            kpts = keypoints.xy[det["idx"]].cpu().numpy()
            if camera_state.esp_settings["skeleton"]:
                skeleton_col = get_rainbow_color(det_i * 0.5, 2.0) if camera_state.esp_settings["gay_mode"] else feature_color("color_skeleton", camera_state.esp_settings.get("color_primary", "#00ff00"), camera_state)
            ...
            if camera_state.esp_settings["head"]:
                nk = kpts[0]
                nx, ny = int(nk[0]), int(nk[1])
                if nx > 0 and ny > 0:
                    head_rad = int((x2 - x1) / 8)
                    head_col = feature_color("color_head", "#ff0000", camera_state)
                    cv2.circle(frame, (nx, ny), max(head_rad, 5), head_col, line_thickness)

    # Draw face boxes
    if camera_state.esp_settings.get("face_boxes") and len(faces) > 0:
        face_col = feature_color("color_face", camera_state.esp_settings.get("color_secondary", "#ff00ff"), camera_state)
    ...
    if camera_state.esp_settings.get("face_blur") and len(faces) > 0:
        k = int(camera_state.esp_settings.get("face_blur_strength", 15))
        if k % 2 == 0:
            k += 1
        k = max(3, k)
        for (fx, fy, fw, fh) in faces:
    ...
    if camera_state.esp_settings.get("face_zoom") and len(faces) > 0:
    ...
    if camera_state.esp_settings.get("hand_skeleton") and hands_landmarks:
        hand_connections = [
        ...
        for pts in hands_landmarks:
            for (a, b) in hand_connections:
                if a < len(pts) and b < len(pts):
                    hand_col = feature_color("color_hand", camera_state.esp_settings.get("color_primary", "#00ff00"), camera_state)
    ...
    stale_keys = [k for k, v in camera_state.smoothed_boxes.items() if current_frame_idx - v["last"] > 30]
    for k in stale_keys:
        camera_state.smoothed_boxes.pop(k, None)
